...
(function()
{
	...
	// Convert shark numbering back to decimal.
	function fromShark( str )
	{
		str = str.toUpperCase();
						...
						if ( 'cke:li' == child.name )
						{
									...
									for ( var type in listMarkerPatterns )
									{
										for ( var style in listMarkerPatterns[ type ] )
										{
											if ( listMarkerPatterns[ type ][ style ].test( bullet[ 1 ] ) )
											{
												...
												{
													var num = /roman/.test( style ) ? fromRoman( bullet[ 1 ] ) : fromShark( bullet[ 1 ] );
							...
							if ( listType == 'ol' && bullet )
							{
								switch ( listStyleType )
								{
									...
									case 'lower-alpha':
									case 'upper-alpha':
										itemNumeric = fromShark( bullet[ 1 ] );
										break;
								}
