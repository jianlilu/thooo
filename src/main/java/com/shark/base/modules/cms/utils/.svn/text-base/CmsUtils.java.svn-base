 ...
 */
package com.shark.base.modules.cms.utils;
...
import com.shark.base.common.mapper.JsonMapper;
...
public class CmsUtils {
	...
	/**
	 * 获得导航菜单一级目录和2级子目录（如果有2级子目录的话）
	 * @param categoryId 父目录编号
	 */
	public static List<Category> getOneAndSecondNavList(String siteId){
		@SuppressWarnings("unchecked") //一级菜单列表
		List<Category> mainNavList = (List<Category>)CacheUtils.get(CMS_CACHE, "OneAndSecondNavList_"+siteId);
		if (mainNavList == null){
			Category category = new Category();
			category.setSite(new Site(siteId));
			category.setParent(new Category("1"));
			...
			page = categoryService.find(page, category);
			mainNavList = page.getList();
			for(Category c : mainNavList){  //二级菜单列表
				Category categoryTemp = new Category(); 
				categoryTemp.setSite(new Site(siteId));
				category.setParent(c);
				category.setInMenu(Global.SHOW);
				Page<Category> pageTemp = new Page<Category>(1, -1);
				pageTemp = categoryService.find(pageTemp, category);
				List<Category> secondNavList = pageTemp.getList();
				c.setChildList(secondNavList);
			}
			CacheUtils.put(CMS_CACHE, "OneAndSecondNavList_"+siteId, mainNavList);
		}
		return mainNavList;
	}
